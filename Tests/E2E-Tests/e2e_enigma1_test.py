...
def test_select_version_enigma_1():
    """Clicks Enigma 1 button, returns assertion that version is Enigma 1"""
    ...
    # Assert that version "Enigma 1" has been selected
    assert driver.find_element(By.XPATH, "/html/body/div[1]/span/h1").text == "Enigma 1"
...
def test_reset():
    ...
    # Assert that previous ciphertext output has been deleted
...
def test_input():
    ...
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[1]").text == "VXMK"
...
def test_physical_input():
    ...
    # Checking for correct ciphertext according to encryption rules
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[1]").text == "VQY"
...
def test_input_with_selected_rotors():
    ...
    # Sending input with virtual keyboard with defaults: reflector ukw-a, starting positions A, A, A
    for _ in range(4):
        driver.find_element(By.ID, "virtualKeyboardT").click()

    # Checking for correct ciphertext according to encryption rules with different rotors
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[1]").text == "HLFZ"
...
def test_select_reflector():
    ...
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[1]").text == "ZAEU"
...
def test_select_starting_positions():
    ...
    Select(driver.find_element(By.ID, "rotor3InitialPosition"))
    driver.find_element(By.ID, "rotor3InitialPositionOptionB").click()
    ...
    # Checking for correct ciphertext according to encryption rules with different reflector
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[1]").text == "ENPF"
...
def test_history_output():
    ...
    # Selecting reflector
    select_reflector = Select(driver.find_element(By.ID, "reflector"))
    select_reflector.select_by_value("ukw-a")
...
def test_add_modify_delete_variant():
    """Adds, modifies and deletes enigma variant,
    returns assertion that added/modified enigma has been deleted"""

    driver.get("http://enigma:8080")

    driver.implicitly_wait(3)

    # Calling function to add enigma TEST variant
    driver.execute_script("add_enigma_variant()")
    # Refreshing page
    driver.refresh()

    # Selects TEST enigma variant
    driver.find_element(By.XPATH, "/html/body/div[1]/nav/ul/li[4]/a").click()
    # Asserts that TEST variant exists and has been selected
    assert driver.find_element(By.XPATH, "/html/body/div[1]/span/h1").text == "TEST"

    sleep(1)

    # Modifies initial positions of rotors 1, 2, 3, 4 and 5
    Select(driver.find_element(By.ID, "rotor1InitialPosition"))
    driver.find_element(By.ID, "rotor1InitialPositionOptionC").click()
    ...
    Select(driver.find_element(By.ID, "rotor3InitialPosition"))
    driver.find_element(By.ID, "rotor3InitialPositionOptionE").click()

    Select(driver.find_element(By.ID, "rotor4InitialPosition"))
    driver.find_element(By.ID, "rotor4InitialPositionOptionN").click()

    Select(driver.find_element(By.ID, "rotor5InitialPosition"))
    driver.find_element(By.ID, "rotor5InitialPositionOptionJ").click()

    # Clicking key in virtual keyboard and sending input to machine
    for x in range(5):
        driver.find_element(By.ID, "virtualKeyboardT").click()

    # Asserts that ciphertext appears and that its output is correct
    assert driver.find_element(By.XPATH, "/html/body/div[2]/div[2]/div/div[2]/p[2]").text == "VBUÄY"

    # Calling function to delete enigma TEST variant
    driver.execute_script("recover_origin_enigma()")
    # Refreshing page
    driver.refresh()

    # Asserting that selected enigma version has defaulted to Enigma 1 after deletion
    assert driver.find_element(By.XPATH, "/html/body/div[1]/span/h1").text == "Enigma 1"
